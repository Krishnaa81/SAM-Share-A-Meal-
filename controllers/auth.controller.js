...
const setTokenCookie = (res, token) => {
  ...
  console.log('Set token cookie with maxAge:', COOKIE_EXPIRY);
};
...
const register = async (req, res) => {
  try {
    ...
    console.log('Creating new user with raw password...');
    const user = new User({
      name,
      email,
      password, // Plain password, will be hashed by the model middleware
      phone,
      role
    });
    ...
    if (user) {
      ...
      console.log('Generated token for new user:', token.substring(0, 20) + '...');
      
      // Set HTTP-only cookie
      setTokenCookie(res, token);
      
      // Send token and user data in response for consistency with login
      res.status(201).json({
        success: true,
        token: token,
        user: {
...
const login = async (req, res) => {
  try {
    ...
    console.log('Login attempt with details:', { email, passwordProvided: !!password });
    ...
    console.log('User found:', { 
      ...
      role: user.role, 
      status: user.status,
      passwordHash: user.password ? user.password.substring(0, 10) + '...' : 'not set'
    });
    ...
    // Check password
    console.log('Checking password match using user.comparePassword method...');
